...
class Alerte_AController extends Controller {
  ...
  {
    $alerte=DB::table('alerte_av')->where('id_Vehicule',$Matricule)->where('Type','assurance')->orderby('id','DESC')->get();
      $Vehicule=DB::table('vehicule')->where('Matricule',$Matricule)->first();
$dateInDB=new DateTime();
$newDate = $dateInDB->format('Y-m-d');
       return view('admin.Alerte_A',['alerte'=> $alerte , 'Vehicule'=> $Vehicule , 'newDate'=>$newDate,'id1'=>$id1]);
  }
...
DB::table('alerte_av')->insert($data);
  ...
  return redirect('/admin/Alerte_A/'. $Matricule.'/'.$id1 )->with('success','Ajout avec succès');
  }
...
$Matricule=DB::table('alerte_av')->where('id',$id)->value('id_Vehicule');
    ...
    return redirect('/admin/Alerte_A/'. $Matricule.'/'.$id1 )->with('success','Modification avec succès');
  ...
  {  $Matricule=DB::table('alerte_av')->where('id',$id)->value('id_Vehicule');
    ...
    return redirect('/admin/Alerte_A/'. $Matricule.'/'.$id1)->with('success','Suppression avec succès');
  }
