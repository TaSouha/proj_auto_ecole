...
class DepenseController extends Controller {
  ...
  public function index($id)
  {
    ...
    $auto_ecole=DB::table('auto_ecole')->where('id',$id_Auto_ecole)->first();
        $depense= DB::table('depense')->where('id_Auto_ecole',$id_Auto_ecole)
            ->orderby('id','DESC')->get();

       return view('admin.Depense',['depense'=> $depense, 'id1'=> $id , 'auto_ecole'=>$auto_ecole ]);
  }
...
DB::table('depense')->insert($data);
  ...
  return redirect('/admin/Depense/'. $id )->with('success','Ajout avec succès');
  }
...
DB::table('depense')->where('id',$id)->update($data);
  return redirect('/admin/Depense/'. $id1 )->with('success','Modification avec succès');
  }
  ...
  return redirect('admin/Depense/'. $id1 )->with('success','Suppression avec succès');
