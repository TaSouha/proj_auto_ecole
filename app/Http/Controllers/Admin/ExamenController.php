...
class ExamenController extends Controller {
  ...
  {$id_Auto_ecole=DB::table('teamwork')->where('id_Personne',$id1)->value('id_Auto_ecole');
    $examen= DB::table('examen')
          ...
          ->where('examen.Type_Exa','Examen théorique')
...
$list=DB::table('candidat')->join('personne','personne.id','=','candidat.id_Personne')->where('candidat.id_Auto_ecole',$id_Auto_ecole)->select('candidat.id','personne.Nom','personne.Prenom')->get();
       ...
       return view('admin.Examen',compact('examen','id1','list'));
  }
...
$data= array('Date_dec' => $Date_dec,
 ...
 'Resultat' => $Resultat,
 'Type_Exa' => 'Examen théorique'
);
DB::table('examen')->insert($data);


  return redirect('/admin/Examen/'. $id1)->with('success','Ajout avec succès');
  }
...
DB::table('examen')->where('id',$id)->update($data);
 ...
 return redirect('/admin/Examen/'. $id1)->with('success','Modification avec succès');
...
DB::table('examen')->where('id',$id)->update($data);
 ...
 return redirect('/admin/Examen/'. $id1)->with('success','Validation avec succès');
...
DB::table('examen')->where('id',$id)->update($data);
 ...
 return redirect('/admin/Examen/'. $id1)->with('success','Invalidation avec succès');
 ...
 return redirect('/admin/Examen/'. $id1)->with('success','Suppression avec succès');
  }
