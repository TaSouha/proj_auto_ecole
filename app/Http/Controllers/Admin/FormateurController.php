...
class FormateurController extends Controller {
 ...
 $list=DB::table('auto_ecole')->groupBy('id')->get();
       return view('admin.Formateur',compact('Administrateur','list','id1'));
  }
...
DB::table('teamwork')->insert($data1);
  ...
  return redirect('/admin/Formateur/'. $id1 )->with('success','Ajout avec succès');
  }
...
DB::table('personne')->where('id',$id)->update($data);
 ...
 return redirect('/admin/Formateur/'. $id1 )->with('success','Modification avec succès');
}
 ...
 return redirect('/admin/Formateur/'. $id1 )->with('success','Suppression avec succès');
  }
