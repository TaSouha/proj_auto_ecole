...
class FormateurController extends Controller {
 ...
 $list=DB::table('auto_ecole')->groupBy('id')->get();
       return view('Formateur.index',compact('Administrateur','list'));
  }
  ...
  {
     $list=DB::table('auto_ecole')->groupBy('id')->get();
    return view('Formateur.create')->with('list', $list);
  }
...
DB::table('teamwork')->insert($data1);
  ...
  return redirect()->action('FormateurController@index')->with('success','Ajout avec succès');
  }
  ...
  $Nom_Auto= DB::table('auto_ecole')->where('id',$id_A)->value('Nom');
             return view('Formateur.show',['administrateur'=> $administrateur, 'Nom_Auto'=>$Nom_Auto ]);
  }
...
$Sexe =$req -> Input('Sexe');
$Adresse =$req -> input("Adresse");
$Mot_de_passe =$req->input("Mot_de_passe");
$email =$req -> input("email");
$Photo =$req->input("Photo");
$data= array('Nom' => $Nom,
 'Prenom' => $Prenom, 
...
DB::table('personne')->where('id',$id)->update($data);
 ...
 return redirect()->action('FormateurController@index')->with('success','Modification avec succès');
}
 ...
 return redirect()->action('FormateurController@index')->with('success','Suppression avec succès');
  }
