...
$newDate = $dateInDB->format('Y-m-d');
       return view('moniteur_part.Alerte_A',['alerte'=> $alerte , 'Vehicule'=> $Vehicule , 'newDate'=>$newDate,'id1'=>$id1]);
  }
...
DB::table('alerte_av')->insert($data);
  ...
  return redirect('/moniteur/Alerte_A/'.$id1 )->with('success','Ajout avec succès');
  }
...
$Matricule=DB::table('alerte_av')->where('id',$id)->value('id_Vehicule');
    ...
    return redirect('/moniteur/Alerte_A/'.$id1 )->with('success','Modification avec succès');
  ...
  {  $Matricule=DB::table('alerte_av')->where('id',$id)->value('id_Vehicule');
    ...
    return redirect('/moniteur/Alerte_A/'.$id1)->with('success','Suppression avec succès');
  }
