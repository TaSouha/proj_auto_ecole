...
$newDate = $dateInDB->format('Y-m-d');
       return view('moniteur_part.Alerte_Vidange',['alerte'=> $alerte , 'Vehicule'=> $Vehicule , 'newDate'=>$newDate,'id1'=>$id1]);
  }
...
DB::table('alerte_vidange')->insert($data);
  ...
  return redirect('/moniteur/Alerte_Vidange/'.$id1 )->with('success','Ajout avec succès');
  }
...
$Matricule=DB::table('alerte_vidange')->where('id',$id)->value('id_Vehicule');
    ...
    return redirect('/moniteur/Alerte_Vidange/'.$id1 )->with('success','modification avec succès');
  ...
  {  $Matricule=DB::table('alerte_vidange')->where('id',$id)->value('id_Vehicule');
    ...
    return redirect('/moniteur/Alerte_Vidange/'.$id1 )->with('success','Suppression avec succès');
  }
