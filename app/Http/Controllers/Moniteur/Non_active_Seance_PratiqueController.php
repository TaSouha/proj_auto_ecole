...
class Non_active_Seance_PratiqueController extends Controller {
  ...
  $id_moniteur=DB::table('teamwork')->where('id_Personne',$id1)->where('Type','moniteur')->value('id');
...
$list=DB::table('candidat')->join('personne','personne.id','=','candidat.id_Personne')->where('candidat.id_Auto_ecole',$id_Auto_ecole)->select('candidat.id','personne.Nom','personne.Prenom')->get();
       ...
       return view('moniteur_part.Non_active_Seance_Pratique',compact('seance_pratique','id1','liste','list'));
  }
...
DB::table('seance_pratique')->insert($data1);
  ...
  return redirect('/moniteur/Non_active_Seance_Pratique/'. $id1)->with('success','Ajout avec succès');
  }
...
DB::table('seance_pratique')->where('id',$id)->update($data1);
 ...
 return redirect('/moniteur/Non_active_Seance_Pratique/'. $id1)->with('success','Modification avec succès');
...
DB::table('facture')->where('id_Candidat',$id_candidat)->update($data1);
 ...
 return redirect('/moniteur/Non_active_Seance_Pratique/'. $id1)->with('success','Validation avec succès');
...
DB::table('seance')->where('id',$id2)->update($data);
 ...
 return redirect('/moniteur/Non_active_Seance_Pratique/'. $id1)->with('success','Invalidation avec succès');
 ...
 return redirect('/moniteur/Non_active_Seance_Pratique/'. $id1)->with('success','Suppression avec succès');
  }
