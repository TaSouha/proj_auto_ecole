...
class MoniteurController extends Controller {
 ...
 $list=DB::table('auto_ecole')->groupBy('id')->get();
       return view('Moniteur.index',compact('Administrateur','list'));
  }
...
DB::table('teamwork')->insert($data1);
  ...
  return redirect()->action('MoniteurController@index')->with('success','Ajout avec succès');
  }
...
DB::table('personne')->where('id',$id)->update($data);
 ...
 return redirect()->action('MoniteurController@index')->with('success','Modification avec succès');
}
 ...
 return redirect()->action('MoniteurController@index')->with('success','Suppression avec succès');
  }
