...
$newDate = $dateInDB->format('Y-m-d');
       ...
       return view('Paiement.index',['Paiement'=> $Paiement, 'id'=> $id,'candidat'=>$candidat,'auto_ecole'=>$auto_ecole , 'newDate'=>$newDate]);
  }
...
DB::table('facture')->where('id_Candidat',$id_candidat)->update($data_facture);
  ...
  return redirect('/index_paiement/'. $id )->with('success','Ajout avec succès');
  }
...
DB::table('paiement')->where('id',$id)->update($data);
  return redirect('/index_paiement/'. $id_Personne )->with('success','Modification avec succès');
  }
  ...
  return redirect('/index_paiement/'. $id_Personne )->with('success','Suppression avec succès');
  }
