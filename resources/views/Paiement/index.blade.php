...
{!! Form::open(['method' => 'DELETE','url' => ['destroy_paiement', $value->id],'style'=>'display:inline']) !!}
...
<form action="/proj_auto_ecole/public/update_paiement/{{ $value->id}}" method="POST" data-parsley-validate>
...
{{ method_field('PATCH') }}
   ...
   <th><textarea name="Designations" class="form-control pull-right" value="{{old('Designations')}}">{{old('Designations')}}</textarea>
      ...
      <a class="btn btn-primary btn-sm" href="{{url('/candidat')}}">
